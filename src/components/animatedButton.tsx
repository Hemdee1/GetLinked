const AnimatedButton = ({ toggle }: { toggle: boolean }) => {
  return (
    <span className="relative grid w-4 h-6 place-content-center">
      <svg
        className={`transition-all duration-500 absolute left-1/2 -translate-x-1/2 -translate-y-1/2 ${
          toggle
            ? "top-full opacity-0 invisible"
            : "top-1/2 visible opacity-100"
        }`}
        width="19"
        height="14"
        viewBox="0 0 19 14"
      ...
      <svg
        className={`transition-all duration-500 absolute left-1/2 -translate-x-1/2 -translate-y-1/2 ${
          !toggle ? "top-0 opacity-0 invisible" : "top-1/2 visible opacity-100"
        }`}
        width="23"
        height="23"
        viewBox="0 0 23 23"
